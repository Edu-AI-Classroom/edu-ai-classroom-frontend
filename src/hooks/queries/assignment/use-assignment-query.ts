...
import type { CreateAssignmentPayload, UpdateAssignmentPayload } from '@/types/class';
...
export function useCreateAssignment(classId?: number) {
	...
	return useMutation({
		mutationFn: (payload: CreateAssignmentPayload) => AssignmentService.createAssignment(payload),
		onSuccess: () => {
...
export function useDeleteAssignment(classId?: number) {
	...
	return useMutation({
		mutationFn: (assignmentId: number) => AssignmentService.deleteAssignment(assignmentId),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: queryKeys.assignment.all });
			if (classId) {
				queryClient.invalidateQueries({
