...
export function CreateAssignmentDialog({
	...
	const [_dueDate, _setDueDate] = useState<string>('');
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="rounded-3xl bg-white shadow-xl max-w-md border border-[#E0DCD5]">
				...
				<form onSubmit={handleSubmit} className="space-y-5 py-4">
					<div>
						<label
							htmlFor="create-assignment-title"
					...
					<div>
						<label
							htmlFor="create-assignment-description"
							className="block text-sm font-semibold text-[#666] mb-2"
						>
					...
					<div>
						<div className="flex items-center justify-between mb-2">
							<label
								htmlFor="create-assignment-grade-level"
								className="text-sm font-semibold text-[#666]"
							>
						...
						<input
							id="create-assignment-grade-level"
							type="range"
							min="1"
							max="5"
							step="1"
					...
					<div>
						<label
							htmlFor="create-assignment-subject"
							className="block text-sm font-semibold text-[#666] mb-2"
						>
							Subject (Optional)
						</label>
						<select
							id="create-assignment-subject"
							value={subjectId ?? ''}
							onChange={(e) => {
