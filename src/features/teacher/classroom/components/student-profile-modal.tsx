...
export function StudentProfileModal({
	...
	const {
		...
		queryFn: () => httpGet<AuthUserResponse>(API_ENDPOINTS.USER.USER_PROFILE(Number(student.userId))),
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="rounded-3xl bg-white shadow-xl max-w-lg border border-[#E0DCD5]">
				...
				<div className="py-4 space-y-6">
					{isLoading && (
						<p className="text-sm text-[#666]">Loading student information...</p>
					)}
					...
					{!isLoading && !error && (
						<>
							<div className="flex items-center gap-4">
								...
								<div>
									<p className="font-sans font-semibold text-lg text-[#333]">
										{displayName}
									</p>
