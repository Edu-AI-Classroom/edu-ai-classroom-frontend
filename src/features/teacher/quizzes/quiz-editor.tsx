			...
			<header className="sticky top-0 z-30 bg-[#FAF9F6]/95 backdrop-blur-sm border-b border-[#E0DCD5]">
				<div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
					...
					<Button
						onClick={onSaveQuiz}
						disabled={isSaving}
						className="rounded-xl bg-gradient-to-r from-[#F5B041] to-[#FA8D3E] text-white border-0 shadow-md hover:shadow-lg transition-all"
					>
			...
			<main className="max-w-6xl mx-auto px-6 py-8 space-y-6">
				...
				<section className="bg-white rounded-2xl border border-[#E0DCD5] shadow-sm overflow-hidden">
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-sm text-[#666]">
									Nhập yêu cầu bằng tiếng Việt để tạo câu hỏi + đáp án.
								</p>
							</div>
						</div>
					</div>
					...
					<div className="p-5 space-y-4">
						...
						<div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
							<div>
								<p className="block text-xs font-semibold text-[#666] mb-1">Tổng số câu</p>
								<Input
									type="number"
							...
							<div>
								<p className="block text-xs font-semibold text-[#666] mb-1">Trắc nghiệm (MCQ)</p>
								<Input
									type="number"
									value={aiMcqCount}
							...
							<div>
								<p className="block text-xs font-semibold text-[#666] mb-1">Tự luận (Essay)</p>
								<Input
									type="number"
									value={aiEssayCount}
							...
							<div>
								<p className="block text-xs font-semibold text-[#666] mb-1">Điểm / câu</p>
								<Input
									type="number"
						...
						{aiGenerated && aiGenerated.length > 0 ? (
							<div className="mt-2 rounded-xl border border-[#E0DCD5] bg-[#F9F8F6] p-4 space-y-3">
								{aiGenerated.slice(0, 10).map((q, idx) => (
									<div
										key={`${q.type}-${q.questionText}`}
										className="rounded-xl bg-white border border-[#E0DCD5] p-3"
									>
										...
										{q.type === 'MCQ' ? (
											<div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
												{q.options.map((opt, i) => (
													<div
														key={`${q.questionText}-${opt}`}
														className={`text-sm px-2 py-1 rounded-lg border ${
															i === q.correctIndex
																? 'border-[#A8D5BA] bg-[#A8D5BA]/15'
																: 'border-[#E0DCD5]'
														}`}
													>
						...
						{aiGenerate.isPending && (
							<div className="mt-4 flex flex-col items-center justify-center p-8 border border-dashed border-[#E0DCD5] rounded-xl bg-[#FAF9F6]">
								...
								<p className="text-sm font-medium text-[#333]">
									AI đang soạn câu hỏi, vui lòng đợi...
								</p>
				...
				<section className="bg-white rounded-2xl border border-[#E0DCD5] shadow-sm p-5">
					<div className="grid md:grid-cols-2 gap-4">
						<div>
							<p className="block text-sm font-semibold text-[#666] mb-2">Title</p>
							<Input
								value={title}
						...
						<div>
							<p className="block text-sm font-semibold text-[#666] mb-2">Document Type</p>
							<select
								value={documentType}
						...
						<div className="md:col-span-2">
							<p className="block text-sm font-semibold text-[#666] mb-2">Description</p>
							<textarea
								value={description}
						...
						<div>
							<p className="block text-sm font-semibold text-[#666] mb-2">Classroom</p>
							<select
								value={classroomId ?? ''}
								onChange={(e) => {
						...
						<div className="grid grid-cols-2 gap-3">
							<div>
								<p className="block text-sm font-semibold text-[#666] mb-2">Time Limit</p>
								<Input
									type="number"
									min={0}
							...
							<div>
								<p className="block text-sm font-semibold text-[#666] mb-2">Total Points</p>
								<Input
									type="number"
									min={0}
									value={totalPoints ?? ''}
						...
						<div className="md:col-span-2">
							<p className="block text-sm font-semibold text-[#666] mb-2">Due date</p>
							<Input
								type="datetime-local"
								value={dueDate}
				...
				<section className="bg-white rounded-2xl border border-[#E0DCD5] shadow-sm overflow-hidden">
					...
					<div className="p-5 space-y-6">
						<div className="rounded-2xl border border-[#E0DCD5] bg-[#FFFCF5] p-4">
							<div className="flex flex-col sm:flex-row sm:items-end gap-3">
								<div className="sm:w-48">
									<p className="block text-sm font-semibold text-[#666] mb-2">Type</p>
									<select
										value={newQType}
								...
								<div className="flex-1">
									<p className="block text-sm font-semibold text-[#666] mb-2">Question text</p>
									<Input
										value={newQText}
								...
								<div className="sm:w-40">
									<p className="block text-sm font-semibold text-[#666] mb-2">Max score</p>
									<Input
										type="number"
										min={0}
										value={newMaxScore}
								...
								<div className="mt-4 space-y-3">
									<div className="grid sm:grid-cols-2 gap-3">
										{optionSlots.map((slot, idx) => (
											<div key={slot}>
												<p className="block text-xs font-semibold text-[#666] mb-1">
													Option {idx + 1}
												</p>
												<Input
									...
									<div className="flex flex-col sm:flex-row sm:items-center gap-3">
										<div>
											<p className="block text-xs font-semibold text-[#666] mb-1">Correct answer</p>
											<select
						...
						<div className="space-y-3">
							<h3 className="font-sans font-bold text-[#333]">
								...
								<div key={q.id} className="rounded-2xl border border-[#E0DCD5] bg-white p-4">
									<div className="flex items-start justify-between gap-3">
										<div className="min-w-0 flex-1">
											...
											<div className="mt-3">
												<p className="block text-xs font-semibold text-[#666] mb-1">Question</p>
												<Input
													defaultValue={q.questionText}
													onBlur={(e) => {
														const value = e.target.value.trim();
														if (value && value !== q.questionText)
															onUpdateQuestion(q, { questionText: value });
													}}
													className="rounded-xl border-[#E0DCD5] bg-[#F9F8F6] focus:bg-white text-[#333]"
												/>
											</div>
											...
											{q.type === 'MCQ' && (
												<div className="mt-3 grid sm:grid-cols-2 gap-3">
													...
													<div>
														...
														>
															{q.options.map((opt, i) => (
																<option key={`${q.id}-correct-${opt}`} value={i}>
																	Option {i + 1}
																</option>
															))}
														</select>
											...
											<div className="mt-3 grid sm:grid-cols-3 gap-3">
												<div className="sm:col-span-1">
													<p className="block text-xs font-semibold text-[#666] mb-1">Max score</p>
													<Input
														type="number"
														min={0}
