...
import { useQuizDetail, useQuizQuestions, useQuizSubmissionsOverview } from '@/hooks/queries/quiz/use-quiz-query';
import {
  Bar,
  BarChart,
  CartesianGrid,
    ...
    <div className="min-h-screen bg-[#FAF9F6] grid-paper">
      <header className="sticky top-0 z-30 bg-[#FAF9F6]/95 backdrop-blur-sm border-b border-[#E0DCD5]">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
          <div className="min-w-0">
            <Link href="/quizzes" className="inline-flex items-center gap-2 text-sm text-[#666] hover:text-[#333]">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <MetricPill label="Time limit" value={quiz.timeLimitMinutes ? `${quiz.timeLimitMinutes} min` : '—'} />
          <MetricPill label="Total points" value={quiz.totalPoints != null ? `${quiz.totalPoints}` : '—'} />
          ...
          <TabsContent value="questions" className="mt-4">
            <div className="bg-white rounded-2xl border border-[#E0DCD5] shadow-sm overflow-hidden">
              <div className="p-5 border-b border-[#E0DCD5]">
                ...
                <p className="text-sm text-[#666]">Questions are ordered as students will see them.</p>
              </div>
                      ...
                      <div className="min-w-0">
                        ...
                        {q.type === 'MCQ' && (
                          <ul className="mt-2 space-y-1 text-sm text-[#666] list-disc pl-5">
                            {q.options.map((opt, i) => (
                              <li key={`${q.id}-${i}`} className={i === q.correctIndex ? 'text-[#256f4b] font-semibold' : ''}>
                                {opt}
                              </li>
                            ))}
                          </ul>
              ...
              <div className="p-4 h-[280px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={submissions?.attemptsByDay ?? []}>
                    ...
                    <Tooltip />
                    <Line type="monotone" dataKey="attempts" stroke="#F5B041" strokeWidth={3} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
