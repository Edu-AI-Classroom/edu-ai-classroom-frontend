...
// Types duplicated from teacher/class-feed.tsx for consistency
...
export default function CourseFeed({ classData }: CourseFeedProps) {
		...
		<div className="max-w-2xl mx-auto space-y-6 pb-20">
						...
						<div className="flex items-center justify-between mt-3 pt-3 border-t border-[#E0DCD5]">
							<div className="flex items-center gap-2">
								...
								<Button
									...
									disabled={createNewsMutation.isPending}
								>
									<ImageIcon className="w-4 h-4 mr-1" /> Attach
								</Button>
								<DropdownMenu>
			...
			{!isLoading && pinnedPosts.length > 0 && (
				<div className="space-y-4">
					<h2 className="font-sans font-semibold text-sm text-[#666] uppercase tracking-wide flex items-center gap-2">
						<Star className="w-4 h-4 text-[#F5B041]" /> Pinned
					</h2>
			...
			{!isLoading && announcements.length === 0 && (
				<div className="text-center py-16">
					...
					<p className="text-sm text-[#999]">Stay tuned for updates from your teacher!</p>
				</div>
			)}
...
function PostCard({
	...
	const addCommentMut = useMutation({
		...
		onSuccess: () => {
			setNewComment('');
			onToggleComments(); // Match teacher's logic to fix refresh/view issue
			invalidateFeed();
		},
	});
	...
	return (
		<>
			<div
				className={`bg-white rounded-2xl shadow-sm border-2 overflow-hidden transition-all ${isPinned ? 'border-[#F5B041]/50' : 'border-[#E0DCD5]'}`}
			>
				<div className="p-5">
						...
						<div className="flex-1">
							<div className="flex items-center gap-2">
								...
								{post.authorRole === 'teacher' && (
									<span className="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-[#F5B041]/10 text-[#F5B041]">
										Teacher
									</span>
								)}
							</div>
					...
					<div className="border-t border-[#E0DCD5] bg-[#FAF9F6] p-4 space-y-3">
						{post.comments.map((comment) => {
							...
							return (
								<div
									key={comment.id}
									className="bg-white rounded-xl p-3 ml-4 shadow-sm border border-[#E0DCD5] relative group"
								>
						...
						<div className="flex items-start gap-2 mt-4 ml-4">
							...
							<div className="flex-1 relative">
								<Textarea
									...
									className="min-h-10 pr-10 resize-none rounded-xl text-sm border-[#E0DCD5] bg-white focus-visible:ring-[#F5B041]"
									onKeyDown={(e) => {
								...
								<button
									...
									className="absolute right-2 bottom-2 p-1.5 text-[#F5B041] hover:text-[#E5A030] disabled:opacity-50"
								>
