...
export const AssignmentService = {
	...
	getAssignments: (params?: PaginationParams) =>
		httpGet<Assignment[]>(
			`${API_ENDPOINTS.ASSIGNMENT.GET_ASSIGNMENTS}${buildQueryString(params)}`,
		),

	getAssignmentsByClassroom: (classId: number) =>
		httpGet<Assignment[]>(
			`${API_ENDPOINTS.ASSIGNMENT.GET_ASSIGNMENTS}?classId=${classId}`,
		),
	...
	updateAssignment: (assignmentId: number, payload: UpdateAssignmentPayload) =>
		httpPut<AssignmentResponse>(
			API_ENDPOINTS.ASSIGNMENT.UPDATE_ASSIGNMENT(assignmentId),
			payload,
		),
